...
const DECIMAL_REFERENCE_STRING = (0)
  ...
  .replaceAll((0).toLocaleString(), '');
export const GROUPING_SEPARATOR = GROUPING_REFERENCE_STRING?.length
  ? GROUPING_REFERENCE_STRING[0]
  : '';
export const DECIMAL_SEPARATOR = DECIMAL_REFERENCE_STRING?.length
  ? DECIMAL_REFERENCE_STRING[0]
  : ''; // Let's hope this edge case never happens 😅
...
export function clampNumbers(num: number, min: number, max: number): number {
