...
const CoinSelection = {
   ...
   * @param {string} coinsPerByte
   */

  setProtocolParameters: (minUTxO, minFeeA, minFeeB, maxTxSize, coinsPerByte) => {
    protocolParameters = {
      minUTxO: minUTxO,
      minFeeA: minFeeA,
      minFeeB: minFeeB,
      maxTxSize: maxTxSize,
      coinsPerByte: coinsPerByte,
    };
  },
  /**
  ...
  randomImprove: (inputs, outputs, limit, preset = []) => {
    ...
    if (utxoSelection.remaining.length > 0) {
      ...
      let dataCost = Loader.Cardano.DataCost.new_coins_per_byte(
        Loader.Cardano.BigNum.from_str(protocolParameters.coinsPerByte)
      );

      // we just use some random address from us for approximating the minOutput required
      const transactionOutput = Loader.Cardano.TransactionOutputBuilder.new()
        .with_address(
      ...
      let minAmount = Loader.Cardano.Value.new(
        Loader.Cardano.min_ada_for_output(transactionOutput, dataCost)
      );

      // This approximation simply doesn't work in most cases
      // let minAmount = Loader.Cardano.Value.new(Loader.Cardano.BigNum.from_str('1800000'))

      let maxFee =
        BigInt(protocolParameters.minFeeA) * BigInt(protocolParameters.maxTxSize) +
...
function randomSelect(utxoSelection, outputAmount, limit, minUTxOValue) {
  ...
  const startingIndex = Math.floor(Math.random() * nbFreeUTxO);
  let utxo = utxoSelection.subset.splice(startingIndex, 1).pop();
...
function splitAmounts(amounts) {
  ...
  splitAmounts.push(
    Loader.Cardano.Value.new(Loader.Cardano.BigNum.from_bytes(amounts.coin().to_bytes()))
  );
