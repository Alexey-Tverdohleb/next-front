...
export const getTXBuilderConfig = async (
  protocolParameters,
  addDatumWitness,
  multiDatum = false
) => {
  ...
  const priceMemFranctionNumerator = Loader.Cardano.BigNum.from_str(
    priceMemFranction.numerator.toString()
  );
  const priceMemFranctionDenominator = Loader.Cardano.BigNum.from_str(
    priceMemFranction.denominator.toString()
  );
  const priceMem = Loader.Cardano.UnitInterval.new(
    priceMemFranctionNumerator,
    priceMemFranctionDenominator
  );

  const priceStepFranction = new Fraction(protocolParameters.priceStep);
  const priceStepFranctionNumerator = Loader.Cardano.BigNum.from_str(
    priceStepFranction.numerator.toString()
  );
  const priceStepFranctionDenominator = Loader.Cardano.BigNum.from_str(
    priceStepFranction.denominator.toString()
  );

  const priceStep = Loader.Cardano.UnitInterval.new(
    ...
    .fee_algo(
      Loader.Cardano.LinearFee.new(
        ...
        Loader.Cardano.BigNum.from_str(
          (Number(protocolParameters.linearFee.minFeeB) + extraFee).toString()
        )
      )
    )
